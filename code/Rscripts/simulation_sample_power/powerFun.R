...
# powerPlot(maxN, Wncp)                                               # Function to plot the power curves and sample sizes for 20% , 50% & 80% power. 
# powerValue(N, Wncp)                                                 # Function to calculate the power for a specific sample size
...
# acePowOrdn(add, com, percent, Nmz, Ndz)                             # Function to calculate power for univariate ordinal twin models
...
powerPlot <- function(maxN, Wncp, pcrit=.05){
	n<- 1:maxN
...
powerValue <- function(N, Wncp, pcrit = .05){
...
dzData <- mxFactor(as.data.frame(dzData), levels = 1:(nTH+1))
	...
	A <-  mxMatrix( type="Symm", nrow=nv, ncol=nv, free=TRUE, values=.3, label="A11", name="A" )
	C <-  mxMatrix( type="Symm", nrow=nv, ncol=nv, free=TRUE, values=.3, label="C11", name="C" )
	E <-  mxMatrix( type="Symm", nrow=nv, ncol=nv, free=TRUE, values=.4, label="E11", name="E" )
