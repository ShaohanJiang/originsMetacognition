...
SAT_path <- function(mzData, dzData, vars){
  ...
  return(list(fitSAT = fitSAT, fitEMO = fitEMO))
  
} # end of func


ACE_path <- function(mzData, dzData, vars, wa=0.5){
  ...
  # Create para
  weighta <- mxMatrix(type = "Full", nrow = 1, ncol = 1, free = FALSE, values = wa, name = "wa")
  ...
  covDZ <- mxAlgebra( expression= wa%x%A+ C, name="cDZ" )
  ...
  # Create Model Objects for Multiple Groups
  pars <- list( meanG, pathA, pathC, pathE, covA, covC, covE, covP, weighta)
  ...
  return(list(fitACE = fitACE, fitAE = fitAE, fitCE = fitCE, fitE = fitE))
  
} # end of func


ADE_path <- function(mzData, dzData, vars){
  ...
  return(list(fitADE, fitAE, fitDE, fitE))
}
